...
def augment_data(df, combine_as_columns = False):
    ...
    if on_other_augmented:
        df_premises = df['sentence1_augmented'].tolist()
        df_hypotheses = df['sentence2_augmented'].tolist()
    else:
...
parser.add_argument("--combine_as_columns", type=bool, help="Whether to combine the augmented data in the same column in the original data or create new columns containing the augmented data.")
