...
for curr_iteration in range(0,max_iteration):
	...
	create_data_for_pretrained_lms(Prepared_data_output_location, label_to_idx_dict, pseudoLabeledSet, tokenizer, 'pseudo',model_type)

	X_pseudo = load_data(Prepared_data_output_location+'X_pseudo.pkl')
	att_mask_pseudo = load_data(Prepared_data_output_location+'att_mask_pseudo.pkl')
	y_pseudo = load_data(Prepared_data_output_location+'y_pseudo.pkl')

	
	y_pseudo = np.asarray(y_pseudo)
	X_pseudo = np.asarray(X_pseudo)
	

	train_model(model_location, model_type, X_pseudo, att_mask_pseudo, y_pseudo, X_valid, att_mask_valid, y_valid, device,batch_size,accumulation_steps,num_epochs,num_classes,report_every, epoch_patience, load=False)
	_, _, _ = test_model(model_location, model_type, X_test, att_mask_test, y_test, device, batch_size,num_classes, curr_NLI_training_base+'test_set_performance_after_pseudo.csv')
	train_model(model_location, model_type, X_train, att_mask_train, y_train, X_valid, att_mask_valid, y_valid, device,batch_size,accumulation_steps,num_epochs,num_classes,report_every, epoch_patience, load=True)
